...
function upstreamUrl(req) {
  ...
  if (!/^https:\/\/[^/]+\.onrender\.com$/.test(baseUrl)) {
    throw new Error('TWM_BASE_URL is not configured for the trip proxy.');
  }
  const pathValue = Array.isArray(req.query.path) ? req.query.path.join('/') : (req.query.path || '');
  const target = new URL(`${baseUrl}/trips${pathValue ? `/${pathValue}` : ''}`);
...
module.exports = async function tripProxy(req, res) {
  ...
  try {
    ...
    console.error('Trip proxy failed.', { error_type: error?.name || 'Error' });
    return res.status(502).json({ detail: 'Trip persistence is temporarily unavailable.' });
  }
};
