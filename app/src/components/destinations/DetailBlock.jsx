...
export default function DetailBlock({ detail }) {
  ...
  if (detail.type === 'cost_breakdown') {
    ...
    const isGroupTotal = items.some(item => item.group);
    return (
      <div className="detail-cost-rows">
        ...
        {items.map(item => (
          <div key={item.label} className="detail-cost-row">
            ...
            <span className="cost-value">≈{formatMoneyRange((item.per_person || item.group).minimum, (item.per_person || item.group).maximum, currency)}</span>
          </div>
        ))}
      </div>
    );
