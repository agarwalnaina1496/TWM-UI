...
// Builder screen: silently advancing PLACES_DRAFT -> DAY_PLAN_DRAFT once
// every necessary input is known, and generating the detailed itinerary.
export function useGuidePlanning(sendTripCommand, navigate) {
  const [generating, setGenerating] = useState(false);

  // Call after any response where Guide owns the trip. Returns the
  // approve_places response when it silently advanced the plan, otherwise
  // null — callers append its message onto the chat log when present.
  async function maybeAdvancePlaces(guideState) {
    if (guideState?.phase !== 'PLACES_DRAFT' || guideState.pending_clarification) return null;
