...
async function seedSelfLed(page) {
...
test('Dashboard/Logistics require Self-Led and uploaded booking state survives refresh', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Upload transport confirmation' }).click();
  ...
  await page.reload();
  await page.getByRole('button', { name: 'Transport' }).click();
  await expect(page.getByText('Uploaded transport confirmation')).toBeVisible();
});
